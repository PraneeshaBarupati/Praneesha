...
public class UserController {
	...
	public ResponseEntity<User> createUser(@RequestBody User user) {
		...
		if (userDAO.get(user.getId()) == null) {
			...
			user.setErrorcode("user already exist with id " + user.getId());
			return new ResponseEntity<User>(user, HttpStatus.OK);
		}
	}
	...
	public ResponseEntity<User> updateUser(@PathVariable("id") String id, @RequestBody User user) {
		...
		if (userDAO.get(id) == null) {
			...
			user = new User();
			user.setErrorcode("user does not exist with id " + user.getId());
	...
	public ResponseEntity<User> deleteUser(@PathVariable("id") String id) {
		...
		if (user == null) {
			...
			user = new User();
			user.setErrorcode("user does not exist");
	...
	public ResponseEntity<User> authenticate(@RequestBody User user, HttpSession session) {
		...
		if (user == null) {
			user = new User();
			user.setErrorcode("Invalid credentials. Please enter valid credentials");
		} else {
