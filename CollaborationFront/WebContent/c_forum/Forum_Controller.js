app.controller('forumController',
		[
				...
				'$log',
				'$rootScope',
				function(forumFactory, $log, $rootScope) {
					var self = this;
					self.forums = [];
					self.forum = {
						forumTitle : '',
					...
					function fetchAllForums() {
						...
						forumFactory.fetchAllForums().then(function(data) {
					...
					function updateForum(forum) {
						self.process = true;
						forumFactory.updateForum(forum).then(function(data) {
					...
					self.deleteForum = function(forumId) {
						// debugger;
						self.process = true;
						forumFactory.deleteForum(forumId).then(function(data) {
							for (var i = 0; i < self.forums.length; i++) {
								if (self.forums[i].forumId === forumId) {
