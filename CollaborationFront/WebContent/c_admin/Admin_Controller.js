app.controller('adminBlogController', [
		'adminFactory',
		'$rootScope',
		'$location',
		function(adminFactory, $rootScope, $location) {
			...
			(function() {
				if ($rootScope.client == undefined || $rootScope.client == null
						|| $rootScope.client.role != 0) {
					$location.path("/");
				}
			})();
			...
			self.approveBlog = function(blogId) {
				self.process = true;
				adminFactory.approveBlog(blogId).then(function(data) {
			...
			self.disapproveBlog = function(blogId) {
				self.process = true;
				adminFactory.disapproveBlog(blogId).then(function(data) {
					var length = self.blogs.length;
					for (var i = 0; i < length; i++) {
						if (self.blogs[i].blog.blogId == blogId) {
							self.blogs.splice(i, 1);
...
app.controller('adminForumController', [ 'adminFactory',
